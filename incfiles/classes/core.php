...
class core
{
    ...
    function __construct()
    {
		...
		$this->lng_detect(); // Tentukan sistem bahasa
    ...
    */
    ...
    -----------------------------------------------------------------
    */
	private function fullkonak()
	{
		if (isset($_GET['w']))
		$_SESSION['fullkonak']= 'web';
		if (isset($_GET['m']))
		$_SESSION['fullkonak']= 'wap';

		if (empty($_SESSION['fullkonak'])){
			if (eregi('windows', self::$user_agent) ||eregi('linux', self::$user_agent) 
			||eregi('bsd', self::$user_agent) ||eregi('x11', self::$user_agent) 
			||eregi('unix', self::$user_agent) ||eregi('unix', self::$user_agent) 
			||eregi('macos', self::$user_agent) ||eregi('macintosh', self::$user_agent)) 
			if (eregi('Windows', self::$user_agent) || (eregi('Linux', self::$user_agent) && !eregi('Android', self::$user_agent)) || eregi('Mac', self::$user_agent))
			$_SESSION['fullkonak'] = 'web';
			elseif (isset(self::$user_agent) && (eregi('(up-browser)|(blackberry)|(windows ce)|windows mobile|symbian|palm|nokia|android|opera mini', self::$user_agent)))
			$_SESSION['fullkonak'] = 'wap';
		}

		if ($_SESSION['fullkonak']== 'web')
		$fullkonak = 'web';
		elseif ($_SESSION['fullkonak']== 'wap')
		$fullkonak = 'wap';
		else
		$fullkonak = 'wap';
		return $fullkonak;
    ...
    */
	...
	/*
    ---------------------------------------------------------------------------------
    Penutupan / Situs mengusir semua pengguna online dan mengarahkan mereka ke halaman keluar
    ---------------------------------------------------------------------------------
    */
