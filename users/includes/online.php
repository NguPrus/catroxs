 ...
 * @developer		agssbuzz@catroxs.org
 */
...
switch ($mod) {
    case 'ip':
        // Daftar IP yang aktif, dengan hits counter
        $ip_array = array_count_values(core::$ip_count);
        $total = count($ip_array);
        if ($start >= $total) {
            //Memperbaiki permintaan ke halaman tidak ada
    ...
    case 'guest':
        // Daftar tamu yang online
        $sql_total = "SELECT COUNT(*) FROM `cms_sessions` WHERE `lastdate` > " . (time() - 600);
        $sql_list = "SELECT * FROM `cms_sessions` WHERE `lastdate` > " . (time() - 600) . " ORDER BY `movings` DESC LIMIT ";
        break;

    case 'history':
        // Sejarah pengunjung selama 2 hari terakhir
        $sql_total = "SELECT COUNT(*) FROM `users` WHERE `lastdate` > " . (time() - 172800 . " AND `lastdate` < " . (time() - 610));
        $sql_list = "SELECT * FROM `users` WHERE `lastdate` > " . (time() - 172800) . " AND `lastdate` < " . (time() - 610) . " ORDER BY `sestime` DESC LIMIT ";
        break;

    default:
        // Daftar Orang yang Online
        $sql_total = "SELECT COUNT(*) FROM `users` WHERE `lastdate` > " . (time() - 600);
        $sql_list = "SELECT * FROM `users` WHERE `lastdate` > " . (time() - 600) . " ORDER BY `name` ASC LIMIT ";
}
