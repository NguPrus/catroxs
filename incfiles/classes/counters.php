...
/* Cчетчик новых постов форума
    $mod = 0 - выводится количество всех и новых (для пользователей) постов (string)
    $mod = 1 - выводится только число новых (int)   
    */
    static function forumCounter($mod = 0){
      $totalPosts = mysql_result(mysql_query("SELECT COUNT(*) FROM `forum_posts` "), 0);
      if (core::$user_id){
        $totalNewPosts = mysql_result(mysql_query("SELECT COUNT(*) FROM `forum_topics` LEFT JOIN `forum_readed` ON `forum_topics`.`id` = `forum_readed`.`topic` AND `forum_readed`.`user_id` = '".core::$user_id."' WHERE (`forum_readed`.`topic` Is Null OR `forum_topics`.`time` > `forum_readed`.`time`) "), 0);
        if ($mod){
          return $totalNewPosts;
        }else{
          return $totalPosts.($totalNewPosts ? ' / <span class="red"><a href="forum/index.php?act=last&amp;unread" title="Непрочитанные">+'.$totalNewPosts.'</a></span>' : '');
        }  
      }else{
        return $totalPosts;
      }
